...
#define side_pneumatics 47
#define top_pneumatic 49
...
void setup()
{
  ...
  pinMode(side_pneumatics, OUTPUT);
  pinMode(top_pneumatic, OUTPUT);
  ...
  digitalWrite(side_pneumatics, LOW);
  digitalWrite(top_pneumatic, LOW);
...
void loop()
{
  ...
  if (xboxConnCheck())
  {
    ...
    // Side Pneumatics Auto
    if (Xbox.getButtonPress(A))
    {
      digitalWrite(side_pneumatics, LOW);
      delay(50);
      digitalWrite(top_pneumatic, HIGH);
    }

    // Side Pneumatics Manual
    if (Xbox.getButtonPress(L2))
    {
      digitalWrite(side_pneumatics, HIGH);
    }
